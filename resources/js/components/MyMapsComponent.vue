<template>
  <div>
    ...
    <map-card-component
      v-for="(map, i) in private_maps"
      :key="map.uuid"
      :map="map"
      :disable_map="i > 0 ? true : false"
    ></map-card-component>
    <div v-if="private_maps.length < 1" class="alert bg-dark">
      You have no maps yet. Create your first map or browse the public ones
      below.
      <button
        type="submit"
    ...
    <div class="row justify-content-center mb-5" v-if="private_maps_total > 15">
      <div class="small">
        {{ private_maps.length }} / {{ private_maps_total }} of your maps are
        shown here
      </div>
    </div>
    <hr class="my-4" />
    ...
    <div class="row justify-content-center mb-5" v-if="public_maps_total > 15">
      <div class="small">
        {{ public_maps.length }} / {{ public_maps_total }} public maps, and a
        lot more private ones
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  created() {
    ...
    axios.get("/api/maps?orderBy=markers_count").then((response) => {
